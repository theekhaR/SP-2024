...
def add_company_industry():
    try:
        ...
        if (not data or
                'industryName' not in data or not data.get('userID')):
...
@companyIndustryListAPI.route('/remove_user_application', methods=['DELETE'])
...
@companyIndustryListAPI.route('/get_user_application', methods=['GET'])
